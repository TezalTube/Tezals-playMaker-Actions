...
namespace HutongGames.PlayMaker.Actions
{
    ...
    [Tooltip("Sends an Event when multiple Keys are pressed.")]
    public class GetKeyComboDown : FsmStateAction
    {
        [RequiredField]
        [Tooltip("The names of the keys to check. Define them as KeyCode values.")]
        public FsmString[] keyNames;  // Array to hold up to 4 keys

        [Tooltip("Event to send if all the keys are pressed.")]
        ...
        public override void OnUpdate()
        {
            ...
            bool allKeysDown = keyNames.All(key => Input.GetKeyDown((KeyCode)Enum.Parse(typeof(KeyCode), key.Value)));

            storeResult.Value = allKeysDown;
